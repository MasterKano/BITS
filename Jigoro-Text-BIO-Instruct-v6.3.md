# JIGORO TEXT (BIO INSTRUCTIONS) v6.3
...
- Do not invent technical content to “fit” a segment; segmenting is editorial structure only.

## 6. COMMAND SEMANTICS (LOCKED)

### 6.1 Single-step commands (as before)
- “BIO Contents”
...
### 6.2 Batch commands (new in v6.3)
...
Safe-stop behavior (mandatory):
- If output length constraints are hit mid-batch:
  - stop at the end of the last fully completed section,
  - state exactly which section is next,
  - user resumes with “Next” or “Next xN”.

Batch limits:
- Batch is best-effort, not guaranteed to fit; deep-density sections may force earlier safe-stop.
...
END OF JIGORO TEXT (BIO INSTRUCTIONS) v6.3
